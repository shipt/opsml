...
function resolveParams(repository: string, name: string, version: string) {
  let providedRepo = repository;
  let providedName = name;
...
function insertRunMetrics(runcard, metrics) {
  if (metrics !== undefined && metrics.length > 0) {
    ...
    $.ajax({
      ...
      success(data) {
        ...
        if (returnedMetrics !== undefined && returnedMetrics.length > 0) {
          ...
          metricBody.innerHTML = "";
          metricBody.innerHTML +=
            '<tr><td><font color="#999">None</font></td><tr>';
        }
      },
      error(xhr, status, error) {
        // eslint-disable-line @typescript-eslint/no-unused-vars
    ...
    document.getElementById("run-metrics-button").onclick =
...
function insertArtifactUris(runcard) {
  if (Object.keys(runcard.artifact_uris).length > 0) {
    ...
    document.getElementById("artifact-button").onclick =
      function artifactToggle() {
        $("#Artifacts").toggle();
      };
...
function insertPlots(runcard) {
  const request = {
    runcard_uri: `${runcard.repository}/${runcard.name}/v${runcard.version}`,
  };

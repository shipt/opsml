...
test('test-buildModelVersionUI', () => {
  ...
  expect(document.getElementById('onnx-uri').href).toBe(`/opsml/files/download/ui?path=test`);
  expect(document.getElementById('model-uri').href).toBe(`/opsml/files/download/ui?path=test`);
  expect(document.getElementById('preprocessor-uri').href).toBe(`/opsml/files/download/ui?path=${fakeData.processor_uris.preprocessor.rpath}`);
  expect(document.getElementById('tokenizer-uri').href).toBe(`/opsml/files/download/ui?path=${fakeData.processor_uris.tokenizer.rpath}`);
  expect(document.getElementById('feature-extractor-uri').href).toBe(`/opsml/files/download/ui?path=${fakeData.processor_uris.feature_extractor.rpath}`);
  expect(document.getElementById('datacard-link').href).toBe(`/opsml/ui?registry=data&uid=test`);
  expect(document.getElementById('runcard-link').href).toBe(`/opsml/ui?registry=run&uid=test`);

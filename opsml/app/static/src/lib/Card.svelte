<script lang="ts">
  ...
  export let defaultClass = "block card card-hover p-4 rounded-2xl w-full dark:bg-surface-700 dark:hover:bg-surface-600";
...
<a class={ defaultClass } href="#">
  <header class="flex items-center ml-0.5 mb-0.5" title="{repository}/{name}">
    <h4 class="text-md truncate font-boing text-smd {hoverColor}">{repository}/{name}</h4>
  </header>

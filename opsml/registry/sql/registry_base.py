...
class SQLRegistryBase:
    ...
    def _check_and_validate_semver(self, card: ArtifactCard) -> CardVersion:
        """
        ...
        version = CardVersion(version=card.version)
        if version.is_full_semver:
            records = self.list_cards(name=card.name, team=card.team, version=version.valid_version)
            if len(records) > 0:
                raise ValueError("Major, minor and patch version combination already exists")
        return version

    def _set_card_version(self, card: ArtifactCard, version_type: VersionType) -> None:
        ...
        if card.version is not None:
            card_version = self._check_and_validate_semver(card=card)
